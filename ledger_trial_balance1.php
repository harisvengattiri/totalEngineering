 ...
 <div style="" id="batch" class="row">
    <div class="col-md-8">
      <div class="box">
        ...
        <div class="box-body">
                   <form role="form" action="<?php echo $baseurl;?>/ledger_trial_balance1" method="post">
...
<!-- row starts here--> 
          ...
          ?>
          <?php
            $sql = "SELECT sum(amount) AS Hold_rcp FROM reciept WHERE status='Hold' AND STR_TO_DATE(voucher.pdate, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y')";
            $query = mysqli_query($conn,$sql);
            $result = mysqli_fetch_array($query);
