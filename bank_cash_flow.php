...
)results ORDER BY STR_TO_DATE(date, '%d/%m/%Y') DESC LIMIT 100";
...
?>
        ...
        <?php
        $sql = "SELECT * FROM (
        (SELECT CONCAT('RPT', id) AS id, sum(amount) as rcp_tot FROM reciept WHERE inward='$bank' AND status='Cleared')
        UNION ALL(SELECT CONCAT('PVR', id) AS id, sum(amount) as vch_tot FROM voucher WHERE inward='$bank' AND status='Cleared')
        UNION ALL(SELECT CONCAT('MIS', id) AS id, sum(total) as mis_tot FROM miscellaneous WHERE bank='$bank')
        UNION ALL(SELECT CONCAT('PTY', id) AS id, sum(amount) as pty_tot FROM petty_voucher WHERE bank='$bank')
        UNION ALL(SELECT CONCAT('RFD', id) AS id, sum(amount) as rfd_tot FROM refund WHERE bank='$bank')
        )results";
        $result = mysqli_query($conn, $sql);
        if (mysqli_num_rows($result) > 0)
		{
        while($row = mysqli_fetch_assoc($result)) {
        $idet=$row["id"];
        if (substr($idet, 0, 3) === 'RPT') { $rcp_tot=$row["rcp_tot"];}
        elseif (substr($idet, 0, 3) === 'PVR') { $vch_tot=$row["rcp_tot"];}
        elseif (substr($idet, 0, 3) === 'MIS') { $mis_tot=$row["rcp_tot"];}
        elseif (substr($idet, 0, 3) === 'PTY') { $pty_tot=$row["rcp_tot"];}
        elseif (substr($idet, 0, 3) === 'RFD') { $rfd_tot=$row["rcp_tot"];}
        }
		}
		$totalcr = $rcp_tot + $mis_tot; 
        $totaldr = $vch_tot + $pty_tot + $rfd_tot;
      ...
      <table width="100%" align="right">
          <tr>
              ...
              <td align="center" width="10%"><b><?php echo custom_money_format("%!i",$balance);?> Dhs</b></td>
          </tr>
        </tfoot>
      </table>
<br/><br/>
