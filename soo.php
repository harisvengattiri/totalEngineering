            ...
            <div class="col-md-12">
                ...
                if ($sts == "cant__edit") { ?>
                    <p><a class="list-group-item b-l-danger">
                            ...
                            <span class="label rounded label danger pos-rlt m-r-xs">
                                <b class="arrow right b-danger pull-in"></b><i class="ion-checkmark"></i></span>
                            <span class="text-danger"> Already delivered, So you cant edit this order</span>
                        </a></p>
                <?php } ?>
            </div>
            ...
            <div class="box-header">
                ...
                <span style="float: right;">
                    ...
                    <!--<a href="<?php // echo $baseurl; 
                                    ?>/sales_order_new" ><button class="btn btn-outline btn-sm rounded b-success text-success"><i class="fa fa-list"></i> View Recent</a></button>-->
                    <?php // } else { 
                    ?>
                    <!--<a href="<?php // echo $baseurl; 
                                    ?>/sales_order_new?view=all" ><button class="btn btn-outline btn-sm rounded b-success text-success"><i class="fa fa-list"></i> View All</a></button>-->
                    <?php // } 
                    ?>
                ...
                <div style="margin-left:5px;">
                    <?php for ($i = 1; $i <= $btn; $i++) { ?>
                        <a href="<?php echo $baseurl; ?>/sales_order_new?list=<?php echo $i; ?>"><button style="margin:2px 0;padding:5px 8px;" class="btn btn-outline btn-sm rounded b-info text-info <?php if ($i == $list) { ?>list-active<?php } ?>">
                                ...
                                if ($btn == $i) {
                                    echo $start + 1; ?> - Latest <?php
                                                } else {
                                                    echo $start + 1; ?> - <?php echo $end;
                                                }
                                    ?>
                        </a></button>
                    <?php } ?>
            ...
            <div class="box-body">
                ...
                <form role="form" action="<?php echo $baseurl; ?>/sales_order_new" method="POST">
                    ...
                    <div class="form-group row">
                        ...
                        <label for="delivery" align="right" class="col-sm-1 form-control-label">Delivery</label>
                        <div class="col-sm-3">
                            <select name="deli_status" class="form-control">
                                <option value="0">ALL</option>
                                <option value="1">Un Delivered</option>
                            </select>
                        </div>
                ...
                if ($_REQUEST) {
                    ...
                    $deli_status = $_REQUEST['deli_status'];

                    $mode = 'Search Mode';
                    $show_date = "[$fdate - $tdate]";
                    
                    $searchEntries = "&fd=$fdate&td=$tdate&cst=$customer&deli=$deli_status";
                } else {
                    $period_sql = "";
                    $cust_sql = "";
                    $mode = 'Recent View';
                ...
                <table class="table m-b-none" data-ui-jp="footable" data-filter="#filter" data-page-size="<?php echo $list_count; ?>">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['view'])) {
                            ...
                            $sql = "
                            SELECT so.*, dn.deliveryAmt
                            FROM sales_order so
                            LEFT JOIN (
                                SELECT order_referance, COALESCE(SUM(total), 0) * 1.05 AS deliveryAmt
                                FROM delivery_note
                                GROUP BY order_referance
                            ) dn ON so.order_referance = dn.order_referance
                            ORDER BY so.id DESC LIMIT 0,250
                            ";
                            
                            // $sql = "SELECT so.*,COALESCE(SUM(dn.total), 0) * 1.05 AS deliveryAmt
                            //         FROM sales_order so
                            //         LEFT JOIN 
                            //         delivery_note AS dn ON so.order_referance = dn.order_referance
                            //         GROUP BY 
                            //         so.order_referance
                            //         ORDER BY id DESC LIMIT 0,250";
                            ...
                            //             c.period AS customer_period, 
                            //             c.Emirate AS customer_Emirate,
                            //             cs.p_name AS site_name,
                            //             sr.name AS salesrep_name,
                            //             COALESCE(SUM(dn.total), 0) * 1.05 AS deliveryAmt
                            //         FROM 
                            //             sales_order AS so
                            //         LEFT JOIN 
                            //             customers AS c ON so.customer = c.id
                            //         LEFT JOIN 
                            //             customer_site AS cs ON so.site = cs.id
                            //         LEFT JOIN 
                            //             customers AS sr ON so.salesrep = sr.id
                            //         LEFT JOIN 
                            //             delivery_note AS dn ON so.order_referance = dn.order_referance
                            //         $period_sql $cust_sql
                            //         GROUP BY 
                            //             so.order_referance
                            //         ORDER BY 
                            //             so.order_referance DESC LIMIT 0,500;
                            //     ";
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $name1 = $row["customer"];
                                $sqlcust = "SELECT name,period,Emirate from customers where id='$name1'";
                                $querycust = mysqli_query($conn, $sqlcust);
                                $fetchcust = mysqli_fetch_array($querycust);
                                $cust = $fetchcust['name'];
                                $period = $fetchcust['period'];
                                $Emirate = $fetchcust['Emirate'];
                            ...
                            if (($deli_status == 1 && $delivered == 'No') || $deli_status != 1) {
                            ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group dropdown dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-scale">
                                                ...
                                                <a class="dropdown-item success" href="<?php echo $baseurl;?>/edit/change_status_po?id=<?php echo $row["id"];?>&status=Active<?php echo $searchEntries;?>">Active</a>
                                                <a class="dropdown-item danger" href="<?php echo $baseurl;?>/edit/change_status_po?id=<?php echo $row["id"];?>&status=Inactive<?php echo $searchEntries;?>">Inactive</a>
                                            </div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <!--<a href="<?php // echo $baseurl; 
                                                        ?>/edit/sales_order?id=<?php // echo $row["id"];
