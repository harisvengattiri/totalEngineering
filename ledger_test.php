 ...
 <div style="" id="batch" class="row">
    <div class="col-md-8">
      <div class="box">
        ...
        <div class="box-body">
                   <form role="form" action="<?php echo $baseurl;?>/ledger_test" method="post">
...
<?php
             ...
             $sqlrcpbal = "SELECT sum(grand) as receipt_grand1 FROM reciept WHERE status='Cleared' AND (STR_TO_DATE(reciept.clearance_date,'%d/%m/%Y') >= STR_TO_DATE('$start_date', '%d/%m/%Y') AND STR_TO_DATE(reciept.clearance_date,'%d/%m/%Y') < STR_TO_DATE('$date1', '%d/%m/%Y'))";
             $queryrcp = mysqli_query($conn, $sqlrcpbal);
             $resultrcp = mysqli_fetch_array($queryrcp);
             $openingrcp = $resultrcp['receipt_grand1'];
            ...
            $sql = "SELECT sum(tbl.total) AS in_advance FROM (
                    ...
                    INNER JOIN
                    reciept_invoice ON reciept.id = reciept_invoice.reciept_id WHERE reciept.type='2' AND status='Cleared' AND (STR_TO_DATE(reciept.clearance_date,'%d/%m/%Y') >= STR_TO_DATE('$start_date', '%d/%m/%Y') AND STR_TO_DATE(reciept.clearance_date,'%d/%m/%Y') < STR_TO_DATE('$date1', '%d/%m/%Y')) AND (STR_TO_DATE(reciept_invoice.date,'%d/%m/%Y') >= STR_TO_DATE('$start_date', '%d/%m/%Y') AND STR_TO_DATE(reciept_invoice.date,'%d/%m/%Y') < STR_TO_DATE('$date1', '%d/%m/%Y')) GROUP BY reciept_invoice.reciept_id
                    ) as tbl";
            $query = mysqli_query($conn,$sql);
            $result = mysqli_fetch_array($query);
            ...
            $opening_vch = $result['opening_vch'];
               ...
               if($petty_bal_op > 0) 
               {
               $actual_opening = ($real_opening+$adv_bal1+$petty_bal_op)-($openingrcp+$credit_note1);
               $actual_borrow = $opening_exp-$opening_vch;
               } else {
               $actual_opening = ($real_opening+$adv_bal1)-($openingrcp+$credit_note1);
...
<!-- row starts here--> 
          ...
          <?php
            $sql = "SELECT * FROM (
                    ...
                    UNION ALL
                    (SELECT CONCAT('RPC', id) AS id, sum(amount) AS Cleared_rcp_enbd FROM reciept WHERE inward='815' AND status='Cleared' AND STR_TO_DATE(reciept.pdate, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y') AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    ...
                    UNION ALL
                    (SELECT CONCAT('PVC', id) AS id, sum(amount) as Cleared_rcp_enbd FROM voucher WHERE inward='815' AND status='Cleared' AND STR_TO_DATE(voucher.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y') AND STR_TO_DATE(voucher.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    )results";
            $query = mysqli_query($conn,$sql);
            ...
            {  
              ...
              if (substr($idet, 0, 3) === 'RPC')
              {
              $enbd_cr2=$row["Cleared_rcp_enbd"];
              }
              elseif (substr($idet, 0, 3) === 'PVR')
              {
              $enbd_dr1=$row["Cleared_rcp_enbd"];
              }
              elseif (substr($idet, 0, 3) === 'PVC')
              {
              $enbd_dr2=$row["Cleared_rcp_enbd"];
              }
            }
          ?>
          <?php
            $sql = "SELECT * FROM (
                    ...
                    UNION ALL
                    (SELECT CONCAT('RPC', id) AS id, sum(amount) AS Cleared_rcp_cbd FROM reciept WHERE inward='816' AND status='Cleared' AND STR_TO_DATE(reciept.pdate, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y') AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    ...
                    UNION ALL
                    (SELECT CONCAT('PVC', id) AS id, sum(amount) as Cleared_rcp_cbd FROM voucher WHERE inward='816' AND status='Cleared' AND STR_TO_DATE(voucher.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y') AND STR_TO_DATE(voucher.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    )results";
            $query = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_array($query))
            {  
              ...
              elseif (substr($idet, 0, 3) === 'PVC')
              {
              $cbd_dr2=$row["Cleared_rcp_cbd"];
              }
            }
          ?>
          <?php
            $sql = "SELECT * FROM (
                    (SELECT CONCAT('RPT', id) AS id, sum(amount) AS Cleared_rcp_credit FROM reciept WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(reciept.pdate, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    (SELECT CONCAT('RPC', id) AS id, sum(amount) AS Cleared_rcp_credit FROM reciept WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(reciept.pdate, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y') AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    (SELECT CONCAT('PVR', id) AS id, sum(amount) as Cleared_rcp_credit FROM voucher WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(voucher.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    (SELECT CONCAT('PVC', id) AS id, sum(amount) as Cleared_rcp_credit FROM voucher WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(voucher.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y') AND STR_TO_DATE(voucher.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    )results";
            $query = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_array($query))
            {  
              ...
              if (substr($idet, 0, 3) === 'RPC')
              {
              $credit_cr2=$row["Cleared_rcp_credit"];
              }
              elseif (substr($idet, 0, 3) === 'PVR')
              {
              $credit_dr1=$row["Cleared_rcp_credit"];
              }
              elseif (substr($idet, 0, 3) === 'PVC')
              {
              $credit_dr2=$row["Cleared_rcp_credit"];
              }
            }
          ?>
          <?php
            $clearence_receipt_enbd = $enbd_cr1-$enbd_cr2;
            $clearence_receipt_cbd = $cbd_cr1-$cbd_cr2;
            $clearence_receipt_credit = $credit_cr1-$credit_cr2;
            $clearence_receipt = $clearence_receipt_enbd+$clearence_receipt_cbd+$clearence_receipt_credit;
            
            $clearence_voucher_enbd = $enbd_dr1-$enbd_dr2;
            $clearence_voucher_cbd = $cbd_dr1-$cbd_dr2;
            $clearence_voucher_credit = $credit_dr1-$credit_dr2;
          ...
          <?php
            $sql = "SELECT * FROM (
                    (SELECT CONCAT('RPT', id) AS id, sum(amount) AS Cleared_rcp_credit FROM reciept WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    (SELECT CONCAT('PVR', id) AS id, sum(amount) as Cleared_rcp_credit FROM voucher WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(voucher.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    (SELECT CONCAT('MSC', id) AS id, sum(total) as Cleared_rcp_credit FROM miscellaneous WHERE bank='1291' AND STR_TO_DATE(miscellaneous.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    UNION ALL
                    (SELECT CONCAT('PTY', id) AS id, sum(amount) as Cleared_rcp_credit FROM petty_voucher WHERE bank='1291' AND STR_TO_DATE(petty_voucher.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    )results";
            $query = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_array($query))
            {  
              ...
              elseif (substr($idet, 0, 3) === 'PTY')
              {
              $credit_dr_petty=$row["Cleared_rcp_credit"];
              }
            }
          ...
          ?>
          <?php
            $receipt_pending = ($actual_opening+$opening_revenue1+$invoice_amount+$adv_bal)-($receipt_total+$credit_note);
