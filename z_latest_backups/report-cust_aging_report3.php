...
if(isset($_POST))
{
    ...
    $date_sql = "AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('01/01/2016', '%d/%m/%Y') AND STR_TO_DATE('$current', '%d/%m/%Y')";
...
if($staff == 'all'){
    ...
    $staff_name = 'ALL';
        ...
        $staff_sql = "AND cs_slmn.id='$staff'";
}
...
<table id="report" class="display nowrap" cellspacing="0" width="100%">
    ...
    while($fetch_cust = mysqli_fetch_array($query_cust))
    {
        ...
        $credit_limit1 = ($fetch_cust['cdt1'] != NULL) ? $fetch_cust['cdt1'] : 0;
        $credit_limit2 = ($fetch_cust['cdt2'] != NULL) ? $fetch_cust['cdt2'] : 0;

        $credit_limit = $credit_limit1 + $credit_limit2;
        ...
        $sql_opn = "
            SELECT (total-(received+credit)) as opn_balance FROM (
                SELECT ROUND(sum(`total`),2) as total,ROUND(sum(`received`),2) as received,ROUND(sum(`credit`),2) as credit FROM (
                    ...
                    WHERE i.grand > 0 AND i.status = '' AND i.customer='$cust_id' AND STR_TO_DATE(i.`date`, '%d/%m/%Y') $cal_opn_date)
                    UNION ALL
                    ...
                    WHERE invoice.grand > 0 AND invoice.status = 'Partially' AND invoice.customer='$cust_id' AND STR_TO_DATE(invoice.`date`, '%d/%m/%Y') $cal_opn_date)
                    ) result
                    ) results";
    ...
    $monthmy = date("m/Y", strtotime ( -$i .'months', strtotime ( $month ) )) ;
        ...
        $sql = "
            SELECT (total-(received+credit)) as balance FROM (
                SELECT ROUND(sum(`total`),2) as total,ROUND(sum(`received`),2) as received,ROUND(sum(`credit`),2) as credit FROM (
                    (SELECT sum(i.grand) as total, 0 as received, 0 as credit FROM invoice i
                    WHERE i.grand > 0 AND i.status = '' AND i.customer='$cust_id' AND i.date LIKE '%$monthmy')
                    UNION ALL
                    (SELECT grand as grand,total10 as received, total2 as credit FROM invoice
                        LEFT JOIN (SELECT invoice, sum(total) as total10 FROM reciept_invoice GROUP BY invoice) as t1 ON invoice.id=t1.invoice
                        LEFT JOIN (SELECT invoice, sum(total) as total2 FROM credit_note GROUP BY invoice) as t2 ON invoice.id=t2.invoice
                    WHERE invoice.grand > 0 AND invoice.status = 'Partially' AND invoice.customer='$cust_id' AND invoice.date LIKE '%$monthmy')
                    ) result
                    ) results";
