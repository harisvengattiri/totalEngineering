...
$tot_amount=($tot_amount != NULL) ? $tot_amount : 0;
...
if($amountsum > $tot_amount) {  $status='failed2'; }  else {
   ...
   $query_del_additionalAcc = mysqli_query($conn,$sql_del_additionalAcc);
    ...
    $sql2 = "UPDATE reciept SET amount='$tamount',pmethod='$pmethod',discount='$discountsum',grand='$grand',clearance_date = '$clearancedate',duedate = '$duedate',ref = '$ref',cat = '36',inward = '$inward',status = '$status1' WHERE id=$id";
    $conn->query($sql2);
...
if($_GET['id'])
{
     ...
     $amount = ($amount != NULL) ? $amount : 0;
        $sql_rfd = "SELECT sum(`amount`) as rfd FROM `refund` WHERE rcp = $receipt";
        $query_rfd=mysqli_query($conn,$sql_rfd);
        $fetch_rfd=mysqli_fetch_array($query_rfd);
        $rfd = $fetch_rfd['rfd'];
        $rfd = ($rfd != NULL) ? $rfd : 0;

        $tot_amount = $amount-$rfd;
...
<!--            <div class="form-group row">
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-4">
                   <input type="hidden" class="form-control" name="tamount" value="<?php echo $amount;?>" placeholder="Amount">
                   <input type="text" class="form-control" name="tot_amount" value="<?php echo $tot_amount;?>" readonly>
              </div>
            </div>
            <div class="form-group row">
