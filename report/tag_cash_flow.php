...
<h2>GROUP CASH FLOW REPORT [<?php echo $fdate;?> - <?php echo $tdate;?>]
<br/><?php echo ucwords($work_type);?> Group: <?php echo ucwords($tag_string);?></h2></center>
<table>
        <thead>
          <tr>
              <th>
...
{
	$sql = "SELECT * FROM (
(SELECT CONCAT('PRC', id) AS id, purchaser as fromid, shop as toid, amount, date, notes FROM purchases WHERE work='project' AND forid IN (SELECT id FROM projects WHERE tags LIKE '%$tag_string%') )
UNION ALL (SELECT CONCAT('PYM', id) AS id, CONCAT(wtype,work) AS purchaser, reciever, amount, date, notes FROM payments WHERE wtype='project' AND work IN (SELECT id FROM projects WHERE tags LIKE '%$tag_string%'))
UNION ALL (SELECT CONCAT('WRI', id) AS id, CONCAT(wtype,work) AS purchaser, collector, paid, duedate, notes FROM work_invoices WHERE paid>0 AND  wtype='project' AND work IN (SELECT id FROM projects WHERE tags LIKE '%$tag_string%'))) results WHERE STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y')  ORDER BY STR_TO_DATE(date, '%d/%m/%Y') ASC";
}
elseif($work=="mtg")
{
	$sql = "SELECT * FROM (
(SELECT CONCAT('PRC', id) AS id, purchaser as fromid, shop as toid, amount, date, notes FROM purchases WHERE work='maintenance' AND forid IN (SELECT id FROM maintenances WHERE tags LIKE '%$tag_string%') )
UNION ALL (SELECT CONCAT('PYM', id) AS id, CONCAT(wtype,work) AS purchaser, reciever, amount, date, notes FROM payments WHERE wtype='maintenance' AND work IN (SELECT id FROM maintenances WHERE tags LIKE '%$tag_string%'))
UNION ALL (SELECT CONCAT('WRI', id) AS id, CONCAT(wtype,work) AS purchaser, collector, paid, duedate, notes FROM work_invoices WHERE paid>0 AND wtype='maintenance' AND work IN (SELECT id FROM maintenances WHERE tags LIKE '%$tag_string%'))) results WHERE STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y')  ORDER BY STR_TO_DATE(date, '%d/%m/%Y') ASC";
}
