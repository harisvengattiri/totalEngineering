...
//          $sql = "SELECT * FROM sales_order where date BETWEEN '$fdate' AND '$tdate' ORDER BY date";
        ...
        ?>
            ...
            ?>
                <tr>
                    <td <?php if($site == NULL) { ?>colspan="8"<?php }else{ ?> colspan="7"<?php } ?>><b></b></td>
                    <td style="text-align: right;"><b><?php echo custom_money_format('%!i', $tot_lpo);?></b></td>
		            <td colspan="4"></td>
                    <?php
                    $mass_lpo = $mass_lpo + $tot_lpo;
                    $tot_lpo = 0;
                    ?>
                </tr>
            <?php } ?>
		...
		<tr>
		    <td <?php if($site == NULL) { ?>colspan="8"<?php }else{ ?> colspan="7"<?php } ?>><b></b></td>
		    <td style="text-align: right;"><b><?php echo custom_money_format('%!i', $tot_amt-$mass_lpo);?></b></td>
		    <td colspan="4"></td>
		</tr>
        </tbody>
        <tr>
