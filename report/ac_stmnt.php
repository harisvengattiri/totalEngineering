...
<!--              <th>
        ...
        $balance=$invtotbal-$getamount;
         ...
         $sql ="SELECT * FROM (
                         ...
                         UNION ALL
                              (SELECT CONCAT('CDT', id) AS id, time, NULL AS po, credit_note.date AS adate, summery AS site, invoice AS lpo, NULL as c_date, NULL as ref,credit_note.total as credit FROM credit_note WHERE customer='$customer' AND STR_TO_DATE(credit_note.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                         UNION ALL
