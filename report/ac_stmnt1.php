...
//                          . " SELECT id,amount,customer,pdate FROM reciept where customer='$customer' ORDER BY date";
        ...
        $invtotbal=$invgran+$invtrans;
        
        $sqlrcp="SELECT sum(amount) AS rcpamount FROM reciept WHERE customer='$customer' AND (STR_TO_DATE(reciept.clearance_date,'%d/%m/%Y') >= STR_TO_DATE('$mindate', '%d/%m/%Y') AND STR_TO_DATE(reciept.clearance_date,'%d/%m/%Y') < STR_TO_DATE('$fdate', '%d/%m/%Y')) ";
        ...
        $balance=$invtotbal-$getamount;
         ...
         $sql ="SELECT * FROM (
                              (SELECT NULL as cdt, lpo AS lpo,invoice.site AS site,invoice.id,time,invoice.transport,NULL AS credit, invoice.grand, invoice.date AS adate FROM invoice WHERE customer='$customer' AND STR_TO_DATE(invoice.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                         UNION ALL
                              (SELECT NULL as cdt, NULL AS lpo,NULL AS site,reciept.id,time,NULL AS tp,reciept.amount, NULL AS credit, reciept.clearance_date AS adate FROM reciept WHERE customer='$customer' AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND status= 'cleared')
                         UNION ALL
                              (SELECT credit_note.id as cdt, NULL AS lpo,NULL AS site,credit_note.id,time,NULL AS tp,credit_note.total, NULL AS credit, credit_note.date AS adate FROM credit_note WHERE customer='$customer' AND STR_TO_DATE(credit_note.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
        ...
        while($row = mysqli_fetch_assoc($result)) {
          ...
          <?php if($sl==1){ ?>
             <tr>
              ...
              <td colspan="2"><b></b></td>
              <td colspan="1" style="text-align: right;"><b><?php echo $bal;?></b></td>
             </tr>
           <?php } ?>  
          <tr>
