...
if(!empty($_POST))
{
    $fdate = $_POST['fdate'];
    $rep = $_POST['staff'];
    if($rep != 0){$staffsql = "AND inv.rep='$rep'";}else{ $staffsql = ""; }
}
$period = date('d/m/Y');
?>
<style type = "text/css">
...
<h2 style="text-align:center;margin-bottom:1px;"><?php // echo strtoupper($status);?>CUSTOMERS ABOVE CREDIT PERIOD</h2>
<table align="center" style="width:90%;">
     <tr>
          <td width="50%">
               <span style="font-size:15px;"></span>
          </td>
          <td width="50%" style="text-align:right"><span style="font-size:15px;"> Date: <?php echo $fdate;?> - <?php echo $period;?></span></td>
     </tr>     
</table>
...
//                         WHERE inv.grand > 0 AND (STR_TO_DATE(inv.date,'%d/%m/%Y') > STR_TO_DATE('$fdate', '%d/%m/%Y')) AND rinv.invoice IS NULL AND cn.invoice IS NULL)
//                         UNION ALL
//                         (SELECT inv.id as invo,inv.date,cs.id as cid,cs.name as customer,cs.cust_type,cs.period,cs1.name as rep FROM invoice inv
//                         JOIN customers cs ON inv.customer=cs.id
...
//                         LEFT JOIN sales_order so ON inv.o_r=so.order_referance
//                         JOIN customers cs1 ON so.salesrep=cs1.id
//                         WHERE inv.grand > 0 AND (STR_TO_DATE(inv.date,'%d/%m/%Y') > STR_TO_DATE('$fdate', '%d/%m/%Y')) AND rpt.status!='Cleared')
//                     ) xss ORDER BY invo ASC
//     			) xmm GROUP BY cid
// 			) xdd WHERE (dt1-period) > 2 ORDER BY (dt1-period) ASC";
...
//                         WHERE inv.grand > 0 AND inv.status != 'Paid' $staffsql AND (STR_TO_DATE(inv.date,'%d/%m/%Y') > STR_TO_DATE('$fdate', '%d/%m/%Y')))
//                         UNION ALL
//                         (SELECT inv.id as invo,inv.date,inv.customer as cid,inv.rep as repr,inv.grand as amt FROM invoice inv
//                         JOIN reciept_invoice rinv ON inv.id=rinv.invoice
//                         JOIN reciept rpt ON rpt.id=rinv.reciept_id
//                         WHERE inv.grand > 0 AND inv.status = 'Paid' $staffsql AND (STR_TO_DATE(inv.date,'%d/%m/%Y') > STR_TO_DATE('$fdate', '%d/%m/%Y')) AND rpt.status!='Cleared')
//                     ) results
...
// 	$sql = "SELECT invo,date,cid,customer,cust_type,period,due,cq_amt,rep,amt,(dt1-period) ocp FROM (
...
//                     ORDER BY invo ASC
//     			) xmm
// 			) xdd WHERE (dt1-period) > 2 GROUP BY cid ORDER BY (dt1-period) ASC";
...
?>
 <table id="tbl1" align="center" style="width:96%;margin-top: 25px;">
        ...
        while($row = mysqli_fetch_assoc($result)) 
        {
            ...
            if($due_date != NULL) {
                $tdays = strtotime(str_replace('/','-',$due_date))-strtotime(str_replace('/','-',$inv_date));
                $days = round($tdays / (60 * 60 * 24));
                $due_over = $days-$credit_period;
                } else { $due_over=NULL; }
            $cq_amt = ($row['cq_amt'] != NULL) ? $row['cq_amt'] : 0;
        ...
        ?>     
             ...
             <tr>
                ...
                <td><?php echo $row['ocp'];?><?php if($due_over != NULL){ echo '/'.$due_over;}?></td>
                <td><?php echo $due_date?></td>
                <td style="text-align:right"><?php echo custom_money_format("%!i",$cq_amt);?></td>
             </tr>
