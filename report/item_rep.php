...
<h2><?php echo strtoupper($status);?>SALESMAN - ITEM ORDER REPORT  </h2>
<h2>     [ <?phpecho $salesrep;?> - <?phpecho $item;?> ]</h2>
...
//	$sql = "SELECT * FROM sales_order where date BETWEEN '$fdate' AND '$tdate'";
        $sql = "SELECT * FROM sales_order WHERE STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND salesrep='$rep'  ORDER BY STR_TO_DATE(date, '%d/%m/%Y') ASC";
