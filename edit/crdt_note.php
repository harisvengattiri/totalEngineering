...
if(isset($_POST['submit']))
{
    ...
    $invoice=$_POST["invoice"];
        if($invoice != NULL) {
            $sql_inv = "SELECT `date` FROM `invoice` WHERE `id`='$invoice'";
            $query_inv = mysqli_query($conn,$sql_inv);
            $result_inv = mysqli_fetch_array($query_inv);
            $inv_date = $result_inv['date'];
        }
...
if($type == 3) {
    ...
    $sql = "UPDATE `credit_note` SET `date`='$date', `invoice`='$invoice', `amount`='$amount', `vat`='$vat', `total`='$total', `summery`='$summery', `description`='$description', `time`='$time' WHERE `id` = '$crdt'";
}
...
if ($conn->query($sql) === TRUE) {
    ...
    $sql_update_additionalRcp = "UPDATE `additionalRcp` SET `invoice`='$invoice',`invoice_date`='$inv_date',`date`='$date',`amount`='-$total' WHERE `entry_id`='$crdt' AND `section`='CNT'";
        ...
        <div class="box-body">
            ...
            <form role="form" action="<?php echo $baseurl;?>/edit/crdt_note?id=<?php echo $crdt;?>" method="post">
              ...
              <div class="form-group row">
                  ...
                  <div class="col-sm-6">
                      <select type="text" class="form-control" name="invoice" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
                        <option value="<?php echo $invoice;?>"><?php echo $invoice;?></option>
                        <?php
                            $sql_cust_inv = "SELECT `id` FROM `invoice` WHERE `customer`='$customer' AND `status` != 'Paid'";
                            $query_cust_inv = mysqli_query($conn,$sql_cust_inv);
                            while($result_cust_inv = mysqli_fetch_array($query_cust_inv))
                            {
                            $cust_inv = $result_cust_inv['id'];
                        ?>
                            <option value="<?php echo $cust_inv;?>"><?php echo $cust_inv;?></option>
                        <?php } ?>
                      </select>
                  </div>
              </div>
