   ...
   $po=$_GET['id'];
    ...
    if ($query1 = $conn->query($sql1))
    {
        ...
        $or = $result1['order_referance'];
            ...
            $sql_check_delivery = "SELECT id AS delivered FROM `delivery_note` WHERE `order_referance`='$or'";
            $query_check_delivery = mysqli_query($conn, $sql_check_delivery);
            $result_check_delivery = mysqli_fetch_array($query_check_delivery);
            if ($result_check_delivery['delivered'] > 0) {
            echo "<script type=\"text/javascript\">" .
                "window.location='$baseurl/sales_order_new?status=cant__edit';" .
                "</script>";
                exit;
            }
        ...
        if($qtn != NULL)
        {
         ...
         while($fetch3 = mysqli_fetch_array($query3)){
            ...
            $qtn_vat = ($qtn_sub_total+$qtn_trans)*0.05;
            $qtn_grand = $qtn_sub_total+$qtn_vat+$qtn_trans;
