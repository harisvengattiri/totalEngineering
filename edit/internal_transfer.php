...
$notes=$_POST["notes"];
$itf=$_POST["itf"];
$sql = "UPDATE `internal_transfers` SET `fromid` = '$fromid', `toid` = '$toid', `date` =  '$date', `amount` =  '$amount', `notes` =  '$notes' WHERE  `id` = $itf";
...
$itf=$_GET["itf"];
	...
	$sql = "SELECT * FROM internal_transfers where id=$itf";
        $result = mysqli_query($conn, $sql);
	...
	<?php }?>
      <div class="box">
        <div class="box-header">
          <h2>Edit Internal Transfer</h2>
          </div>
        <div class="box-divider m-a-0"></div>
        <div class="box-body">
          <form role="form" action="<?php echo $baseurl;?>/edit/internal_transfer" method="post">

<div class="form-group row">
               <input name="itf"  type="text" value="<?php echo $itf;?>" hidden="hidden">
              <label for="fromid" class="col-sm-2 form-control-label" >From</label>
              <div class="col-sm-4">
               <select name="fromid" id="fromid" placeholder="fromid" class="form-control select2" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
				<?php 
				$sql = "SELECT id,name FROM customers where id=$fromid";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
				while($row = mysqli_fetch_assoc($result)) 
              ...
              <label for="toid" align="right" class="col-sm-2 form-control-label">To</label>
              <div class="col-sm-4"> 
                                <select name="toid" id="toid" placeholder="toid" class="form-control select2" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
				<?php 
				$sql = "SELECT id,name FROM customers where id=$toid";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
				while($row = mysqli_fetch_assoc($result)) 
