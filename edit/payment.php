...
$pym=$_POST["pym"];
$sql = "UPDATE `payments` SET `wtype` =  '$wtype', `work` =  '$for', `division` = '$division', `reciever` = '$reciever',`date` =  '$date', `method` = '$method', `amount` =  '$amount', `notes` =  '$notes' WHERE  `id` =$pym";
...
$pym=$_GET["pym"];
	...
	$sql = "SELECT * FROM payments where id=$pym";
        $result = mysqli_query($conn, $sql);
  ...
  <div class="row">
	...
	<?php }?>
      <div class="box">
        ...
        <div class="box-body">
          <form name="editpurchase" role="form" action="<?php echo $baseurl;?>/edit/payment" method="post">
...
<div class="col-sm-4">
    <select name="division" id="subcatsSelect" placeholder="Division" class="form-control select2">
            ...
            <div class="form-group row">
               <input name="pym"  type="text" value="<?php echo $pym;?>" hidden="hidden">
              <label for="customer" class="col-sm-2 form-control-label">Reciever</label>
              <div class="col-sm-4">
				<select name="reciever" id="reciever" placeholder="reciever" class="form-control select2" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
				<?php  
				$sql = "SELECT id,name FROM customers where id=$reciever";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
				while($row = mysqli_fetch_assoc($result)) 
              ...
              <div class="col-sm-4"><input type="text" value="<?php echo $date;?>" name="date" id="date" placeholder="Date" class="form-control has-value" data-ui-jp="datetimepicker" data-ui-options="{
             format: 'DD/MM/YYYY',
            icons: {
              time: 'fa fa-clock-o',
              date: 'fa fa-calendar',
