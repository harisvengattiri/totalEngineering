...
$sql = "UPDATE `petty_cash_new` SET `date`='$date',`staff`='$staff',`amount`='$amount1',`vat`='$vat1',`total`='$total1' WHERE id='$id'";

if ($conn->query($sql) === TRUE) {
      $status="success";
      $last_id = $conn->insert_id;
    
    $sql_dlt = "DELETE FROM petty_item_new WHERE petty=$id";
    ...
    for($i=0;$i<$count;$i++)
    {
     ...
     $vat[$i] = ($vat[$i] != NULL) ? $vat[$i] : 0;
     $total[$i] = $amount[$i] + $vat[$i];
     $sql1 = "INSERT INTO `petty_item_new` (`petty`,`date`,`staff`,`type`,`sub`,`description`,`amount`,`vat`,`total`) 
     VALUES ('$id', '$date', '$staff', '$cat[$i]', '$sub[$i]', '$des[$i]', '$amount[$i]', '$vat[$i]', '$total[$i]')";
     $conn->query($sql1); 
    }
...
if($_GET['id'])
     {
     $id=$_GET['id'];
     $sql="SELECT * FROM petty_cash_new where id = '$id'";
   ...
   $sql1 = "SELECT sum(total) AS total FROM petty_cash_new WHERE staff='$staff'";
  ...
  <div class="row">
	...
	<?php }?>
      <div class="box">
        ...
        <div class="box-body">
          <form role="form" action="<?php echo $baseurl;?>/edit/petty_cash_new?id=<?php echo $id;?>" method="post">
            ...
            icons: {
              ...
              <div class="form-group row">
                         <label for="endd" align="center" class="col-sm-3 form-control-label"><b>Account</b></label>
                         <label for="endd" align="center" class="col-sm-2 form-control-label"><b>Sub Account</b></label>
                         <label for="endd" align="center" class="col-sm-3 form-control-label"><b>Description</b></label>
                         <label for="endd" align="center" class="col-sm-2 form-control-label"><b>Amount</b></label>
                         <label for="endd" align="center" class="col-sm-1 form-control-label"><b>VAT</b></label>
               </div>
            
            <?php 
               $sql1="SELECT * FROM petty_item_new where petty='$id'";
            ...
            <div class="col-sm-3">
                <select name="cat[]" id="cat<?php echo $count;?>" required class="form-control select2" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
                <?php 
				$sql = "SELECT * FROM expense_categories";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
				?><option value="<?php echo $cat;?>"><?php echo $category;?></option><?php
            ...
            <script type="text/javascript">
                 $(document).ready(function() {
                  $("#cat<?php echo $count;?>").change(function() {
                    var cat_id = $(this).val();
                    if(cat_id != "") {
                      $.ajax({
                        ...
                        success:function(response) {
                          var resp = $.trim(response);
                          $("#sub_cat<?php echo $count;?>").html(resp);
                        }
                      });
                    } else {
                      $("#sub_cat<?php echo $count;?>").html("<option value=''>------- Select --------</option>");
                    }
                  });
                });
            </script>    
              <div class="col-sm-2">
                <select name="sub[]" id="sub_cat<?php echo $count;?>" class="form-control select2">
                    <option value="<?php echo $sub;?>"><?php echo $sub_name;?></option>
                </select>
              </div>
              <div class="col-sm-3">
