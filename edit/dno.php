...
<div class="app-body">
  ...
  if (isset($_POST['submit'])) {
    if (isset($_SESSION['userid'])) {
      ...
      function compareDeliveryAndOrder($items, $reqquantity, $prevquantity, $thisquantity, $baseurl) {
        $itemAssoc = [];
        $delivery_flag = 0;
        ...
        for($i = 0; $i < sizeof($items); $i++) {
          ...
          $olddelivery = $prevquantity[$i];

          $currentOrder = $olddelivery+$order;
      
          if (isset($itemAssoc[$item])) {
            $itemAssoc[$item]['order'] += $olddelivery;
            $itemAssoc[$item]['delivery'] += $delivery;
              if($itemAssoc[$item]['delivery'] > $itemAssoc[$item]['order']) {
                $delivery_flag++;
              }
          } else {
            $itemAssoc[$item] = ['order' => $currentOrder, 'delivery' => $delivery];
              if($itemAssoc[$item]['delivery'] > $itemAssoc[$item]['order']) {
                $delivery_flag++;
              }
          }
        ...
        if($delivery_flag > 0) {
          echo '<script>window.location.href = "' .$baseurl. '/edit/dno?status=failed3"</script>';
          exit();
        }
      }

    compareDeliveryAndOrder($items, $reqquantity, $prevquantity, $thisquantity, $baseurl);
      ...
      $sqlinvo = "SELECT ROUND(SUM(total), 2) AS grand FROM `delivery_note` WHERE `customer` = '$customer' AND `id` != '$id'";
      ...
      if ($curr_bal > $limit & $limit != 0) {
        echo "<script type=\"text/javascript\">" .
          "window.location='$baseurl/edit/dno?status=failed4';" .
          "</script>";
      } else {
        ...
        $n = sizeof($items);
          ...
          if ($check == 'notmatch') {
            $sql = "UPDATE delivery_note SET order_referance = '$order_referance', date = '$date', customer = '$customer', customersite = '$customersite', vehicle = '$vehicle', driver = '$driver', vehicleWeight = '$vehicleWeight', allowedWeight = '$allowedWeight', currentWeight = '$currentWeight', note = '$note', transport = '$transport' WHERE id='$id'";
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $sql_po = "SELECT `qtn` AS qtnId FROM `sales_order` WHERE `order_referance`='$or'";
      ...
      <div class="col-md-12">
        ...
        <div class="box">
          ...
          <div class="box-body">
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-4">
                  ...
                  <select name="order_referance" class="form-control" readonly>
                    ...
                    // {
                    ?>
                    <!--<option value="<?php // echo $row["order_referance"]; 
                                        ?>"><?php // echo $row["order_referance"];
                                                                                  ?>/<?php // echo $row["lpo"];
                                                                                                                            ?></option>-->
                    <?php
                    // }} 
            ...
            icons: {
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-4">
                  <select name="customer" class="form-control" id="customer" readonly>
                    ...
                    // {
                    ?>
                    <option value="<?php echo $customer; ?>">[CST-<?php echo $customer;?>] <?php echo $cust; ?></option>
                    <?php
                    // while($row = mysqli_fetch_assoc($result)) 
                    // {
                    ?>
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-4" id="veh">
                  <select name="vehicle" id="vehicleSelect" class="form-control select2-multiple" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
                    <?php
            ...
            <div class="form-group row">
              <label align="" class="col-sm-2 form-control-label">Transportation</label>
              <div class="col-sm-4">
                <select name="transport" class="form-control">
                <?php 
                  $sql = "SELECT * FROM `quotation_transportation` WHERE `quotation_id`='$qtnId'";
                  $result = mysqli_query($conn, $sql);
                  if (mysqli_num_rows($result) > 0) {
                    // if($transport == '0') { echo '<option value="">SELECT</option>';}
                    echo '<option value="">SELECT</option>';
                    while($row = mysqli_fetch_assoc($result)) {
                     $transportation = $row["transportation"];
                  ?>
                    <option value="<?php echo $transportation;?>" <?php if ($transportation == $transport) echo "selected"; ?>><?php echo $transportation;?></option>
                <?php } } ?>
                </select>
              </div>
            </div>
                ...
                ?>
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-sm-2">
                        ...
                        <input type="hidden" class="form-control" name="item[]" value="<?php echo $item; ?>" id="delItem_<?php echo $rowNo;?>" placeholder="Item">
                        ...
                        <input type="hidden" value="<?php echo $weight;?>" id="itemWeight_<?php echo $rowNo;?>">
                      </div>
                      ...
                      <div class="col-sm-2">
                        <input type="number" min="0" step="any" class="form-control" name="thisquantity[]" id="qnt_<?php echo $rowNo;?>" value="<?php echo $thisquantity; ?>">
              ...
              <div class="row">
                <div class="col-md-8">
                  ...
                  <div class="row">
                    <div class="col-sm-4">
                      <input style="text-align: center;" type="text" class="col-sm-4 form-control" name="vehicleWeight" id="vehicleWeight" value="<?php echo $vehicleWeight; ?>" readonly>
                    </div>
                    <div class="col-sm-4">
                      <input style="text-align: center;" type="text" class="col-sm-4 form-control" name="allowedWeight" id="allowedWeight" value="<?php echo $allowedWeight; ?>" readonly>
                    </div>
                    <div class="col-sm-4">
                      <input style="text-align: center;" type="text" class="col-sm-4 form-control" name="currentWeight" id="currentWeight" value="<?php echo $currentWeight; ?>" readonly>
                    </div>
                  </div>
