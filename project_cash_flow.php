...
<div class="padding">
  <div class="box">
    <div class="box-header">
        <?php
        $prj=$_GET['prj'];
        $sql2 = "SELECT name FROM projects where id=$prj";
        ...
        ?>
	<span style="float: left;"><h2>Project Cash Flow Statement [<?php echo $projectname;?>]</h2></span> 
    </div><br/>
    <div class="box-body">
    ...
    <div>
      <table class="table m-b-none" data-ui-jp="footable" data-filter="#filter" data-page-size="500">
		...
		<?php
                ...
                $staff=$_GET['staff'];
		$sql = "SELECT * FROM (
(SELECT CONCAT('PRC', id) AS id, purchaser, shop, amount, date, notes FROM purchases WHERE work='project' and forid=$prj)
UNION ALL (SELECT CONCAT('PYM', id) AS id, CONCAT(wtype,work) AS purchaser, reciever, amount, date, notes FROM payments WHERE wtype='project' and work=$prj)
UNION ALL (SELECT CONCAT('WRI', id) AS id, CONCAT(wtype,work) AS purchaser, collector, paid, duedate, notes FROM work_invoices WHERE wtype='project' and work=$prj and paid>0)
