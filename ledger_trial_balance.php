 ...
 <div style="" id="batch" class="row">
    <div class="col-md-8">
      <div class="box">
        ...
        <div class="box-body">
                   <form role="form" action="<?php echo $baseurl;?>/ledger_trial_balance" method="post">
...
<!-- row starts here--> 
          ...
          <?php
            $sql = "SELECT * FROM (
                    ...
                    UNION ALL (SELECT CONCAT('PVR', id) AS id, sum(amount) as Cleared_rcp_credit FROM voucher WHERE inward='1291' AND status='Cleared' AND STR_TO_DATE(voucher.date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y'))
                    )results";
            $query = mysqli_query($conn,$sql);
          ...
          $expense_vat = ($expense_vat != NULL) ? $expense_vat : 0;
         ...
         $voucher_discount = ($voucher_discount != NULL) ? $voucher_discount : 0;
