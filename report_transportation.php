...
<div class="padding">
    ...
    <div class="col-md-8">
     ...
     <button id="godriver" type="button" class="btn btn-success">Driver Report</button>
        </div></div></div></div> 
          ...
          <form role="form" action="<?php echo $baseurl;?>/report/vehicle" method="post" onsubmit="target_popup(this)">
            ...
            <div class="form-group row m-t-md">
              <div align="right" class="col-sm-offset-2 col-sm-12">
                ...
                <button type="submit" formaction="<?php echo $cdn_url;?>/reports/convert_report_veh" class="btn btn-sm btn-outline rounded b-success text-success">Print</button>
              </div>
            </div>
          </form>
        </div>
  ...
  <div style="display: none;" id="driver" class="row">
    <div class="col-md-9">
      <div class="box">
        <div class="box-header">
          <h2>Driver Report</h2>
          ...
          <form role="form" action="<?php echo $baseurl;?>/report/driver" method="post" onsubmit="target_popup(this)">
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-3">
               <select name="driver" class="form-control">
				<?php 
				$sql = "SELECT cs.name,cs.id FROM `customers` cs
                        INNER JOIN `driverType` dt ON cs.id = dt.driver
                        WHERE cs.type='Driver'";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
				while($row = mysqli_fetch_assoc($result)) 
            ...
            <div class="form-group row m-t-md">
              <div align="right" class="col-sm-offset-2 col-sm-12">
                ...
                <!--<button name="print" type="submit" class="btn btn-sm btn-outline rounded b-info text-info">Print</button>-->
                <button name="submit" type="submit" class="btn btn-sm btn-outline rounded b-success text-success">View</button>
                <button name="submit" type="submit" formaction="<?php echo $baseurl;?>/report/driver_new" class="btn btn-sm btn-outline rounded b-success text-success">Print Excel</button>
                <button type="submit" formaction="<?php echo $cdn_url;?>/reports/convert_report_driver" class="btn btn-sm btn-outline rounded b-success text-success">Print</button>
              </div>
            </div>
          </form>
        </div>
