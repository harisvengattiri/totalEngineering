...
<h2 style="text-align:center;margin-bottom:1px;"><?php // echo strtoupper($status);?>PROFIT & LOSS</h2>
...
<table id="tbl1" align="center" style="width:90%;">
        ...
        <?php
            $sql_main_cat = "SELECT * FROM `expense_main_categories` WHERE `id` > 5";
...
<!--SUB SECTION ENDS HERE -->
        ...
        <?php
            $pl = array_sum($tot_credit) - array_sum($tot_debit);
            if($pl > 0) {$situation = 'Profit';} else {$situation = 'Loss';$pl=-($pl);}
        ?>
        <tr>
            <td colspan="1"></td>
            <td colspan="3" style="text-align:center;"><b><?php echo $situation;?></b></td>
            <td style="text-align:right;"><b><?php echo custom_money_format('%!i', $pl);?></b></td>
        </tr>
