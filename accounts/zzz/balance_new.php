...
<table id="tbl1" align="center" style="width:90%;">
        ...
        <?php
            ...
            //                 `section`='RCP' AND
            //                 STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND
            //                 `date` != ''
            //               ";
            
            // $sql_in_bank = "
            //                 SELECT sum(currentInBank) AS currentInBank FROM (
            //                 (SELECT sum(grand) AS currentInBank FROM `reciept` WHERE `type` = '' AND
            //                 STR_TO_DATE(`clearance_date`, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND
            //                 `clearance_date` != '')
            //                 UNION ALL
            //                 (SELECT sum(reciept_invoice.total) as total FROM reciept
            //                 INNER JOIN reciept_invoice ON reciept.id = reciept_invoice.reciept_id
            //                 WHERE reciept.type='2' AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
            //                 )result
            ...
            $sql_in_bank = "
                            SELECT sum(currentInBank) AS currentInBank FROM (
                            (SELECT sum(grand) AS currentInBank FROM `reciept` WHERE
                            STR_TO_DATE(`clearance_date`, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND
                            `clearance_date` != '')
                            )result
            ...
            $sql_in_credit = "SELECT sum(-amount) AS currentInCredit FROM `additionalRcp` WHERE
                            `section`='CNT' AND
                            STR_TO_DATE(`date`, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND
                            `date` != ''
                           ";
        ...
        <?php
            $sqlReceivables = "SELECT sum(Receivables) AS Receivables FROM (
                                (SELECT sum(amount) AS Receivables FROM `additionalRcp`
            ...
            $sql_adv = "SELECT (sum(grand)-sum(total)) AS advanceBalance FROM (
                    SELECT reciept.grand as grand,sum(reciept_invoice.total) as total FROM reciept
                    INNER JOIN reciept_invoice ON reciept.id = reciept_invoice.reciept_id
                    WHERE reciept.type='2' AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y')  AND reciept.clearance_date != ''
                        AND STR_TO_DATE(reciept_invoice.invoice_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y')
                    GROUP BY reciept_invoice.reciept_id
                    ) as tbl";
            $query_adv = mysqli_query($conn,$sql_adv);
            $result_adv = mysqli_fetch_array($query_adv);
            $advanceBalance = $result_adv['advanceBalance'];
            $tot_credit[] = $advanceBalance;
            $tot_debit[] = 0;
            
            $sql_adv = "SELECT (sum(grand)-sum(total)) AS advanceBalancePrev FROM (
                    SELECT reciept.grand as grand,sum(reciept_invoice.total) as total FROM reciept
                    INNER JOIN reciept_invoice ON reciept.id = reciept_invoice.reciept_id
                    WHERE reciept.type='2' AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') < STR_TO_DATE('$fdate', '%d/%m/%Y') AND reciept.clearance_date != ''
                        AND STR_TO_DATE(reciept_invoice.invoice_date, '%d/%m/%Y') <  STR_TO_DATE('$fdate', '%d/%m/%Y')
                    GROUP BY reciept_invoice.reciept_id
                    ) as tbl";
            $query_adv = mysqli_query($conn,$sql_adv);
            $result_adv = mysqli_fetch_array($query_adv);
            $advanceBalancePrev = $result_adv['advanceBalancePrev'];
            $tot_credit[] = $advanceBalancePrev;
            $tot_debit[] = 0;
        ?>
                ...
                <tr>
                  <td><b></b></td>
                  <td><b></td>
                  <td style="font-size:18px;">ADVANCE BALANCE BEFORE</td>
                      <td></td>
                      <td style="font-size:18px;"><?php echo custom_money_format('%!i', $advanceBalancePrev);?></td>
                </tr>
                ...
                <tr>
                  <td><b></b></td>
                  <td><b></td>
                  <td style="font-size:18px;">IN CREDIT</td>
