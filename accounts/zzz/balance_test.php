...
<table id="tbl1" align="center" style="width:90%;">
        ...
        <?php    
            $sql_find_opening = "SELECT sum(amount) AS pendingInPrevious FROM `receivables` WHERE
        ...
        <?php
            $sql_current_sale = "SELECT sum(amount) AS currentSale FROM `receivables` WHERE
                                `section`='INV' AND
        ...
        <?php
            ...
            $sql_in_bank = "
                            ...
                            (SELECT sum(amount) AS currentInBank FROM `receivables_reciept` WHERE `type` = '1' AND
                            STR_TO_DATE(`cdate`, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y') AND
                            `cdate` != '')
                            UNION ALL
                            (SELECT sum(ABS(receivables.amount)) as total FROM receivables_reciept
                            INNER JOIN receivables ON receivables_reciept.id = receivables.entry_id
                            WHERE receivables_reciept.type='2' AND STR_TO_DATE(receivables_reciept.cdate, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                            )result
            ...
            $sql_in_credit = "SELECT sum(-amount) AS currentInCredit FROM `receivables` WHERE
                            `section`='CNT' AND
        ...
        <?php
            $sql_sundry_debtors = "SELECT sum(oldReceivables) AS sundryDebtors FROM (
                            (SELECT sum(amount) AS oldReceivables FROM `receivables` WHERE
        ...
        <?php
            $sqlReceivables = "SELECT sum(Receivables) AS Receivables FROM (
                                (SELECT sum(amount) AS Receivables FROM `receivables`
            ...
            $sqlPdc = "SELECT sum(-amount) AS pdcAmount FROM `receivables` WHERE
                        `section`='RCP' AND
            ...
            //         WHERE reciept.type='2' AND STR_TO_DATE(reciept.clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$date1', '%d/%m/%Y') AND STR_TO_DATE('$date2', '%d/%m/%Y')
