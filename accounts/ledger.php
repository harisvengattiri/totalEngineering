...
<table align="center" style="width:90%;">
     <tr>
          ...
          <td width="50%" style="text-align:left;border:0;">
              <button onclick="toggleColumns()">Hide/Show Openings</button>
          </td>
          
          <td width="50%" style="text-align:right;border:0;"><span style="font-size:15px;"> Date: <?php echo $fdate;?> - <?php echo $tdate;?></span></td>
     </tr>     
</table>
 
 
 
<table id="trialBalanceTable" align="center" style="width:90%;">
        <thead>
          <!--<tr>-->
          <!--    <th colspan="3"></th>-->
          <!--    <th>Debit</th>-->
          ...
          <tr>
              ...
              <!--</th>-->
              <th style="text-align:center;">
                  Opening <?php echo $year;?><br>Debit
              </th>
              <th style="text-align:center;">
                  Opening <?php echo $year;?><br>Credit
              </th>
              <th>
                  Debit
              </th>
        ...
        <?php
            ...
            if ($main_cat == 1 || $main_cat == 2 || $main_cat == 7 || $main_cat == 8) {
                ...
                while($result_calc = mysqli_fetch_array($query_calc)) {
                    ...
                    $sql_amt_main = "SELECT ROUND(SUM(db_amt), 2) - ROUND(SUM(cd_amt), 2) AS amount FROM (
                                    (SELECT ROUND(SUM(amount), 2) as cd_amt, '' as db_amt FROM `jv_items` WHERE `type` = 'credit' AND `cat` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                                    UNION ALL
                                    (SELECT '' as cd_amt, ROUND(SUM(amount), 2) as db_amt FROM `jv_items` WHERE `type` = 'debit' AND `cat` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                                    UNION ALL
                                    (SELECT '' as cd_amt, ROUND(SUM(grand), 2) as db_amt FROM `pv` WHERE `category` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                                    
                                    UNION ALL
                                    (SELECT ROUND(SUM(grand), 2) as cd_amt, '' as db_amt FROM `pv` WHERE `bank` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                                    
                                    UNION ALL
                                    (SELECT '' as cd_amt, ROUND(SUM(amount), 2) as db_amt FROM `reciept` WHERE `cat` = '$categ' AND STR_TO_DATE(clearance_date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                
                                    UNION ALL
                                    (SELECT ROUND(SUM(total), 2) as cd_amt, '' as db_amt FROM `invoice` WHERE `cat` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                                    
                                    UNION ALL
                                    (SELECT ROUND(SUM(amount), 2) as cd_amt, '' as db_amt FROM `refund` WHERE `cat` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                                    
                                    UNION ALL
                                    (SELECT ROUND(SUM(-amount), 2) as cd_amt, '' as db_amt FROM `credit_note` WHERE `cat` = '$categ' AND STR_TO_DATE(date, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y'))
                            ...
                            $sql_amt_main .= ") result";
                                ...
                                // Specially Designed by Developer
                                    $sql_opnCat = "SELECT ROUND(SUM(amount), 2) as opnCat FROM `account_openings` WHERE `cat` = '$categ' AND `year`='$year'";
...
<script>
  function toggleColumns() {
    ...
    var columnIndex1 = 3; // Change this to the index of the first column you want to toggle
    var columnIndex2 = 4; // Change this to the index of the second column you want to toggle

    for (var i = 0; i < table.rows.length; i++) {
      var row = table.rows[i];
      
      // Toggle visibility for the first column
      var cell1 = row.cells[columnIndex1];
      ...
      // Toggle visibility for the second column
      var cell2 = row.cells[columnIndex2];
