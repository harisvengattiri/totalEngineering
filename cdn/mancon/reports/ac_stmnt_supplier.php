...
// $html = file_get_contents('http://mancon.gulfit.in/mpdf/ac_stmnt_supplier-pdf?fd='.$fdate.'&td='.$tdate.'&cst='.$customer);
// include("../includes/mpdf/mpdf.php");
// $mpdf=new mPDF('utf-8', 'A4', 0, '', 10, 10, 30, 0, 0, 0);

$html = file_get_contents($baseurl.'/mpdf/ac_stmnt_supplier-pdf?fd='.$fdate.'&td='.$tdate.'&cst='.$customer);
...
$mpdf->WriteHTML($html);
$mpdf->Output('Supplier Statement #'.$customer.'.pdf', 'I');
exit;
?>