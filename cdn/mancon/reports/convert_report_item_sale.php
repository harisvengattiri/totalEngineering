...
// $html = file_get_contents('http://mancon.gulfit.in/mpdf/report_item_sale-pdf?fd='.$fdate.'&td='.$tdate.'&sales_rep='.$sr.'&item='.$item);
// include("../includes/mpdf/mpdf.php");
// $mpdf=new mPDF('utf-8', 'A4', 0, '', 10, 10, 30, 0, 0, 0);

$html = file_get_contents($baseurl.'/mpdf/report_item_sale-pdf?fd='.$fdate.'&td='.$tdate.'&sales_rep='.$sr.'&item='.$item);
...
$mpdf->WriteHTML($html);
$mpdf->Output('Item Sale Report.pdf', 'I');
exit;
?>