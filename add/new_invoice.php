...
$sql = "INSERT INTO `invoice` (`token`, `customer`, `site`, `rep`, `cat`, `sub`, `lpo`, `o_r`, `date`, `total`, `vat`, `grand`, `transport`, `bank_details`, `time`) 
VALUES ('$token', '$customer', '$site', '$rep', '40', '191', '$lpo', '$or', '$date', '$subtotal', '$vat', '$grand', '$transportation', '$bank_details', '$time')";
...
VALUES ('$last_id', '$dn[$i]', '$item[$i]', '$pdate[$i]', '$reqquantity[$i]', '$unit[$i]', '$total[$i]')";
$conn->query($sql1);
  ...
  <div class="row">
	...
	<?php }?>
      <div class="box">
        ...
        <div class="box-body">
          <form role="form" action="<?php echo $baseurl;?>/add/new_invoice" method="post">
            <div class="form-group row">
            <label for="customer" class="col-sm-3 form-control-label">Customer</label>
              <div class="col-sm-6">
		        <select name="customer" id="customer" class="form-control" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">
                  <?php 
                              $customer=$_POST['customer'];
                              $site=$_POST['site'];
          ...
          $cust=$fetchcust['name'];
				?>
				<option value="<?php echo $cst;?>">[CST-<?php echo $cst;?>] <?php echo $cust;?></option>
				<?php 
				} }
				?>
                </select>		
              ...
              <div class="col-sm-6">
                   <input type="text" name="date" value="<?php echo $date;?>" id="date" required placeholder="Date" <?php echo $readonly;?> class="form-control has-value" data-ui-jp="datetimepicker" data-ui-options="{
            format: 'DD/MM/YYYY',
            icons: {
              time: 'fa fa-clock-o',
              date: 'fa fa-calendar',
        ...
        ?>
             ...
             <form role="form" action="<?php echo $baseurl;?>/add/new_invoice" method="post">
                    ...
                    <select name="dn[]" multiple id="multiple" class="form-control select2-multiple" multiple data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">     
                         <?php  $sql = "SELECT id FROM delivery_note where order_referance='$so' AND invoiced='' AND `date` != 0 AND total > 0 AND confirmation=1";
                          $result = mysqli_query($conn, $sql);
                          if (mysqli_num_rows($result) > 0) 
                          {
                          while($row = mysqli_fetch_assoc($result)) 
