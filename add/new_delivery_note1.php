...
$sql = "INSERT INTO `delivery_note5` (`id`,`order_referance`,`rep`,`customer`, `customersite`, `lpo`, `date`, `vehicle`,`driver`,`note`,`transport`) 
...
if ($conn->query($sql) === TRUE) {
    $status="success";
       ...
       $sql1 = "INSERT INTO `delivery_item5` (`delivery_id`,`order_referance`, `item`, `comment`, `date`, `reqquantity`, `thisquantity`, `price`,`amt`, `bundle`,`pdate`,`coc`,`batch`) 
...
$conn->query($sql1);
       ...
       $sql2 = "UPDATE delivery_note5 SET total = $amttotal WHERE id = $last_id";
  ...
  <div class="row">
	...
	<?php } else if($status=="failed1") {?>
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form role="form" action="<?php echo $baseurl;?>/add/new_delivery_note1" method="post">
    ...
    if(isset($_POST['submit2'])) {
         ...
         if($status == 'Rejected')
              {
              header("Location: $baseurl/add/new_delivery_note1?status=failed_due_Rejection_from_ADMIN");
              }
         
         if($bal >= $limit & $limit!=NULL)
              {
              header("Location: $baseurl/add/new_delivery_note1?status=failed_due_to_credit_excess");
              }
...
//       echo "<option value='" . $row['site'] . "'>" . $row['site'] . "</option>";
    ...
    <form role="form" action="<?php echo $baseurl;?>/add/new_delivery_note1" method="post">
            ...
            <div class="form-group row">
               <div class="col-sm-8">
                   <?php 
                   $sql = "SELECT id FROM delivery_note5 ORDER BY id DESC LIMIT 1";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
				while($row = mysqli_fetch_assoc($result)) 
