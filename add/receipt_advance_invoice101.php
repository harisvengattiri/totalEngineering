...
if($amountsum > $tamount) {  $status='failed2'; }  else {
    ...
    $sql1 = "INSERT INTO `reciept_invoice` (`reciept_id`,`date`,`invoice`, `amount`, `adjust`, `total`) 
VALUES ('$id','$pdate','$invoice[$i]', '$amount[$i]', '$discount[$i]', '$total[$i]')";
     $conn->query($sql1); 
    }
    $sql2 = "UPDATE reciept SET amount='$tamount',pmethod='$pmethod',discount='$discountsum',grand='$grand',clearance_date = '$clearancedate',duedate = '$duedate',inward = '$inward',status = '$status1' WHERE id=$id";
    $conn->query($sql2);
...
if($_GET['id'])
{
     ...
     $inward = $fetchrcp['inward'];
        $sqlbank="SELECT name FROM customers where id='$inward'";
        $querybank=mysqli_query($conn,$sqlbank);
        $fetchbank=mysqli_fetch_array($querybank);
        $inward_bank=$fetchbank['name'];
...
<!--            <div class="form-group row">
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-4">
                   <input type="text" class="form-control" name="tamount" value="<?php echo $amount;?>" placeholder="Amount" readonly>
              </div>
            </div>
            <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            ...
                            <?php     
                            $sql = "SELECT * FROM customers WHERE type='Bank' ORDER BY id DESC";
            				$result = mysqli_query($conn, $sql);
