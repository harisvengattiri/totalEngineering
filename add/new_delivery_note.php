...
// if($lmt == 0) { $lmt=NULL; }
    ...
    echo "<script type=\"text/javascript\">".
         "window.location='$baseurl/add/new_delivery_note?status=failed4';".
         "</script>";
    }
    else {
...
$sql = "INSERT INTO `delivery_note` (`id`,`token`,`order_referance`,`rep`,`customer`, `customersite`, `lpo`, `date`, `vehicle`,`driver`,`note`,`transport`) 
VALUES ('$dno','$token','$order_referance','$rep','$customer', '$customersite', '$lpo', '$date', '$vehicle', '$driver', '$note', '$transport')";
        ...
        ?> <option value="<?php echo $customer;?>"><?php echo $cust;?></option> <?php
            ...
            <div class="form-group row m-t-md">
             ...
             <?php if(isset($_POST['submit1']))
             {
                  ...
                  $trans=$fetchinvo['total_transport'];
                  $trans=($trans != NULL) ? $trans : 0;
                  $invoamt=$invoamt-$trans;
                  $invoamt=$invoamt*1.05;
                  $invoamt = $invoamt+$trans;

                  $sqlrpt="SELECT sum(grand) AS amount from reciept where customer = $customer AND status = 'Cleared' AND type != 1";
...
<div class="padding" style="padding: 0rem 1.5rem;">
      ...
      <?php
       ...
       ?> 
               ...
               <?php
                 ...
                 if (mysqli_num_rows($result) > 0) 
                  ...
                  while($row = mysqli_fetch_assoc($result)) 
                  {
                                     ...
                                     $quantityreq=($quantityreq != NULL) ? $quantityreq : 0;
           ...
           <div class="form-group row">
               ...
               <div class="col-sm-1">
                    <input type="number" min="1" step="any" class="form-control" name="thisquantity[]" placeholder="Qnt" id="this<?php echo $id;?>">
              </div>
         ...
         <div class="form-group row">
              ...
              <div class="col-sm-2">
                   <select class="form-control" name="batch[]" id="batch<?php echo $id+60;?>">
                         <?php                                
                                        $sql1="SELECT batch FROM batches_lots
                                        WHERE batch NOT IN (SELECT batch FROM delivery_item) AND item='".$item."'
                                        UNION
               ...
               <!--<div id="quantity<?php // echo $id+60;?>"></div>-->
               
                       <div class="col-sm-2">
                       <input type="text" class="form-control" name="quantity[]" id="quantity<?php echo $id+60;?>" value="<?php echo $quantity;?>" readonly>
                       </div>
                      ...
                      <div class="col-sm-1">
                       ...
                       <div class="col-sm-1">
                            <input type="number" min="1" step="any" class="form-control" name="thisquantity[]" placeholder="Qnt" id="this1<?php echo $id;?>">
                      </div>
                      
                      <div class="col-sm-2">
                        <input type="text" class="form-control" name="coc[]" id="coc<?php echo $id+60;?>" value="<?php echo $coc;?>" id="endd" readonly>
                      </div>
      </div> 
           <style>
               hr { 
 ...
 <script type="text/javascript">
 $(document).ready(function() {
  $("#batch<?php echo $id+60;?>").change(function() {
    var country_id = $(this).val();
    if(country_id != "") {
      $.ajax({
        url:"getquantity",
        ...
        success:function(response) {
          var resp = $.trim(response);
          $("#quantity<?php echo $id+60;?>").val(resp);
        }
      });
    } else {
      $("#quantity<?php echo $id+60;?>").html("<option value=''>------- Select --------</option>");
    }
  });
});
</script>
 ...
 $(document).ready(function() {
  $("#batch<?php echo $id+60;?>").change(function() {
    ...
    if(country_id != "") {
      $.ajax({
        ...
        success:function(response) {
          var resp = $.trim(response);
          $("#coc<?php echo $id+60;?>").val(resp);
        }
      });
    } else {
      $("#coc<?php echo $id+60;?>").html("<option value=''>------- Select --------</option>");
    }
  });
});
</script>
  ...
  <script>
        $(document).ready(function(){
           ...
           var f=$('#quantity<?php echo $id+60;?>').val();
