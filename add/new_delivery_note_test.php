...
if($lmt == 0){$lmt=NULL;}
    ...
    if($curr_bal > $lmt & $lmt!=NULL)
    {
        echo 'This is not possible';
        header("Location: $baseurl/add/new_delivery_note_test?status=failed4");
    }
    ...
    // else
    // {
    //     echo $amttotal123.'<br/>'.$crdt_bal.'<br/>'.$lmt.'<br/>'.$curr_bal;
  ...
  <div class="row">
	...
	<?php } else if($status=="failed1") {?>
      ...
      <div class="box">
        ...
        <div class="box-body">
          <form role="form" action="<?php echo $baseurl;?>/add/new_delivery_note_test" method="post">
    ...
    if(isset($_POST['submit2'])) {
         ...
         if($status == 'Rejected')
              {
              header("Location: $baseurl/add/new_delivery_note_test?status=failed_due_Rejection_from_ADMIN");
              }
         
         if($bal >= $limit & $limit!=NULL)
              {
              header("Location: $baseurl/add/new_delivery_note_test?status=failed_due_to_credit_excess");
              }
