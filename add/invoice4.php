...
$vat=$_POST["vat"];
$grand=$_POST["grand"];
$sql = "INSERT INTO `invoice` (`customer`, `site`,`lpo`,`o_r`, `date`, `total`, `vat`, `grand`) 
VALUES ('$customer', '$site', '$lpo', '$or', '$date', '$subtotal', '$vat', '$grand')";
            ...
            <div class="form-group row m-t-md">
             ...
             <form role="form" action="<?php echo $baseurl;?>/add/invoice" method="post">
                    ...
                    <select name="so" class="form-control select2" placeholder="Item" data-ui-jp="select2" data-ui-options="{theme: 'bootstrap'}">     
                         <?php  $sql = "SELECT order_referance FROM sales_order where customer='$customer' AND site='$site'";
				$result = mysqli_query($conn, $sql);
				if (mysqli_num_rows($result) > 0) 
				{
                                while($row = mysqli_fetch_assoc($result)) 
				{
                                  ...
                                  ?><option value="<?php echo $so;?>"><?php echo $so;?></option><?php
                                }}
                        ?>
                    </select>
                    </div></div>
